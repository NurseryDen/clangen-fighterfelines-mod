...
class LifeGenPatrolScreen(Screens):
    ...
    date_icon = pygame.transform.scale(pygame.image.load("resources/images/buttons/flirt.png").convert_alpha(),
                                      (100, 100))
    sex_icon = pygame.transform.scale(pygame.image.load("resources/images/buttons/button_cum.png").convert_alpha(),
                                      (100, 100))
    ...
    def handle_choose_cats_events(self, event, doubleclick=False):
        ...
        if "sex_icon" in self.elements:
                if event.ui_element == self.elements['sex_icon']:
                    self.change_screen("patrol screen5")
                    self.elements['sex_icon'].disable()
    ...
    def update_button(self):
        ...
        if self.patrol_stage == 'choose_cats':
            ...
            if game.clan.game_mode != 'classic':
                ...
                # ff_mod edit: allows apprentices to go on dates!
                if game.clan.your_cat.moons >= 6:
                    self.elements['date_icon'].enable()
                self.elements['sex_icon'].enable()
    ...
    def open_choose_cats_screen(self):
        ...
        self.elements['your_cat'] = UIImageButton(scale(pygame.Rect((886, 1120), (68, 68))), "",
                                              ...
                                              , manager=MANAGER)
        self.elements['your_cat'].disable()
        self.elements['sex_icon'] = UIImageButton(scale(pygame.Rect((646, 1270), (68, 68))), "",
                                             object_id="#cum_button"
                                             , manager=MANAGER)
        self.elements['sex_icon'].disable()
