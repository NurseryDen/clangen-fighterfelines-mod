...
#ff_mod to_do: this might conflict with incest additions.. 
    @staticmethod
    ...
    def handle_zero_moon_pregnant(cat: Cat, other_cat=None, clan=game.clan):
        ...
        # ff_mod to_do: copy teen pregnancy anncment to mpreg code
        ...
        # ff_mod edit: adds custom announcement for underage pregnancies
        under_ages = ["kitten", "adolescent"]
        if pregnant_cat.age in under_ages:
            text = choice(Pregnancy_Events.PREGNANT_STRINGS["teen_pregnancy_announcement"])
        else:
            text = choice(Pregnancy_Events.PREGNANT_STRINGS["announcement"])
        # ff_mod edit: sets severity to major for all young pregnancies
            if clan.game_mode != 'classic':
                severity = random.choices(["minor", "major"], [3, 1], k=1)
                if pregnant_cat.age in under_ages:
                    severity[0] = "major"
    ...
    def check_if_can_have_kits(cat, single_parentage, allow_affair):
        ...
        # ff_mod edit: allows kittens and adolescents to have kits (sorry, newborns!)
        # however, restricting kittens under 5 moons from having cats for realism
        not_correct_age = cat.age in ['newborn'] or cat.moons < 5
    ...
    # ff_mod to_do: i think these control the chances to cancel out a pregnancy before it can start
    # it would be a good idea to decrease the chances of underage cats having kits even further
    @staticmethod
    def get_balanced_kit_chance(first_parent: Cat, second_parent: Cat, affair, clan) -> int:
        ...
        # ff_mod to_do: might also mess with incest.. idk
        # 'INBREED' counter
